...
function App() {
  ...
  const fetchTasks = async () => {
    try {
      //const response = await axios.get("http://10.66.90.225:8081/api/tasks"); //di ZeroTier pribadi, run powershel pakai: $env:HOST="0.0.0.0"; npm start
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-xl shadow-xl p-6">
        ...
        <p className="text-center text-slate-500 mb-6 mt-2">
          <span className="font-bold text-blue-600">{pendingTasks}</span> tugas
          tersisa
        </p>
        ...
        {filteredTasks.length === 0 && (
          <p className="text-center text-slate-400 mt-6 italic text-sm">
